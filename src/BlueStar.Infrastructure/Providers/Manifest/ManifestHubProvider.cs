...
public sealed class ManifestHubProvider : IManifestProvider
{
    ...
    public async Task<IReadOnlyList<ManifestArtifact>> DiscoverManifestsAsync(uint appId, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            if (_keyRepository != null)
            {
                ...
                {
                    await _keyRepository.RegisterKeysAsync(keysToRegister, "ManifestHub JSON", ct).ConfigureAwait(false);
                }

                // Also fetch lua script in background to get any additional keys
                _ = Task.Run(async () =>
                {
                    try
                    {
                        var luaUrl = $"https://raw.githubusercontent.com/SSMGAlt/ManifestHub2/{appId}/{appId}.lua";
                        using var luaResp = await _http.GetAsync(luaUrl, CancellationToken.None).ConfigureAwait(false);
                        if (luaResp.IsSuccessStatusCode)
                        {
                            var lua = await luaResp.Content.ReadAsStringAsync(CancellationToken.None).ConfigureAwait(false);
                            await _keyRepository.ImportFromLuaAsync(lua, CancellationToken.None).ConfigureAwait(false);
                        }
                    }
                    catch { }
    ...
    public async Task<string> DownloadManifestAsync(uint depotId, ulong manifestId, string targetDirectory, uint appId = 0, CancellationToken ct = default)
    {
        ...
        // If AppId is known, SSMGAlt branch is the primary candidate
        if (appId > 0)
        {
            candidateUrls.Add($"https://raw.githubusercontent.com/SSMGAlt/ManifestHub2/{appId}/{depotId}_{manifestId}.manifest");
        }

        // Flat repository mirror indexed directly by depotId_manifestId on main branch
        ...
        foreach (var url in candidateUrls)
        {
            ...
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    var cachedPath = await _cacheService.StoreManifestAsync(depotId, manifestId, stream, ct).ConfigureAwait(false);
